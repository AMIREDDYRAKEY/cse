...
// We will receive the PDF file buffer in memory and upload it directly to MongoDB
...
app.post("/api/upload", authMiddleware, (req, res) => {
  upload.single("pdf")(req, res, (err) => {
    ...
    if (!gridfsBucket) return res.status(500).json({ error: "Database not ready. Please try again in a moment." });

    const uniqueSuffix = Date.now() + "-" + Math.round(Math.random() * 1e9);
    const sanitizedName = req.file.originalname.replace(/[^a-zA-Z0-9.\-_]/g, "_");
    const filename = uniqueSuffix + "-" + sanitizedName;

    // Open a streaming tunnel to MongoDB GridFS
    ...
    // Write the actual file bytes to MongoDB and close the stream
    uploadStream.end(req.file.buffer);
  });
});

// File Retrieval Endpoint (GridFS)
app.get("/api/files/:filename", async (req, res) => {
  try {
    if (!gridfsBucket) {
      return res.status(500).json({ error: "Database not fully initialized yet." });
    }
    ...
    if (file[0].contentType === "application/pdf" || file[0].contentType === "binary/octet-stream") {
